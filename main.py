...
def new_game():
    ...
    letter_list.update({'':answer_length})
...
def check_letter():
    ...
    logging.info(letterGuessed)

    letter_array_guessed = list(session['current_correct'])
    ...
    for i in range (0,word_length):
        if letter_array[i] == letterGuessed:
            letter_array_guessed[i] = letterGuessed
            letterTrue = True
        elif i == word_length - 1 and letterTrue == False:
            session['wrong_guess'] += 1


    session['current_correct'] = "".join(letter_array_guessed)    #Combines back the list of letters
